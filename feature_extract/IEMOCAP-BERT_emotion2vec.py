"""
Extract BERT + emotion2vec features for IEMOCAP
emotion2vec is specifically trained for speech emotion recognition
"""
import warnings
...
def main():
    ...
    process_dataset(
        IEMOCAP_TRAIN_PATH,
        f"{OUTPUT_PATH}IEMOCAP_BERT_emotion2vec_train.pkl"
    )

    # Process validation set
    process_dataset(
        IEMOCAP_VAL_PATH,
        f"{OUTPUT_PATH}IEMOCAP_BERT_emotion2vec_val.pkl"
    )

    # Process test set
    process_dataset(
        IEMOCAP_TEST_PATH,
        f"{OUTPUT_PATH}IEMOCAP_BERT_emotion2vec_test.pkl"
    )
