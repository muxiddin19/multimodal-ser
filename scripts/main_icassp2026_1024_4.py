...
def train_and_evaluate(
    ...
    print(f"\n{'='*60}")
    print(f"FINAL RESULTS ({config.num_runs} runs)")
    print(f"{'='*60}")
    ...
    summary['confusion_matrix'] = avg_conf_matrix.tolist()

    # Per-class metrics (from last run)
    summary['per_class'] = all_results[-1]['per_class']
...
def main():
    ...
    if args.ablation:
        ...
        results = train_and_evaluate(config, train_dataset, val_dataset)
